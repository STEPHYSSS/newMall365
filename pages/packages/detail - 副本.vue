	...
	<view class="w-100 h-100" :class="mainStyle">
		...
		<view style="padding: 30rpx 40rpx; padding-bottom: 100rpx;background-color: #f3f0f0;">
			...
			<view class="font-size-base text-color-base">
				售卖时间：{{ BeneInfo.StartTime }}~{{ BeneInfo.EndTime }}
			</view>
			<view class="font-size-base text-color-base" style="margin-bottom: 30rpx;">
				购买限制：无限制
			</view>
			<view class="font-size-sm text-color-assist pre-line">
				<div v-for="(item,index) in BeneTypeItem" :key="index" style="display: flex;padding: 5px 0;">
					<p style="flex: 1 ;"> {{item.BeneDesc}} </p>
					<p style="width: 50px ;color: #D6A874;" v-if="item.FrequType!='5'"> x{{item.FrequNum}} </p>
					<p style="width: 50px ;color: #D6A874;" v-else> 不限 </p>
				</div>
			</view>
		</view>
	...
	methods: {
		async getInfo(){//获取详情
			...
			try {
				...
				this.BenePayMode = Data.BenePayMode;
