				...
				<div class="homeInfo">
					...
					<div class="backgroundF homeOrderRow">
						...
						<uni-grid :column="5" @change="toGrid" :show-border="false" style="color:#2c3e50">
							<uni-grid-item :index="1" v-if="IsOpenSpread!=0">
								<view class="grid-item-box">
									<div class="iconfont icon-shenqingtuanchang"></div>
									<div v-if="LeaderAudit == '1'|| LeaderAudit == '0'">团长信息</div>
									<div v-else>申请团长</div>
								</view>
							</uni-grid-item>
							<uni-grid-item :index="5" v-if="IsOpenSpread!=0">
								<view class="grid-item-box">
							...
							<uni-grid-item :index="3" v-if="IsOpenSpread!=0">
								<view class="grid-item-box">
					...
					<div class="cardWei">
						...
						<div>
							<adCell v-if="CardType!=='0'" text="会员卡解绑" @click="UnBindCard" />
						</div>
						<view>
							<adCell text="每日签到" @click="$Router.push('/pages/vip/signIn/signIn')"></adCell>
						</view>
					</div>
		...
		methods: {
			...
			async getInfo() {
				...
				try {
					...
					if (data.Data.CardBase) {
						...
						this.ImgUrl = GetBaseImgUrl() + data.Data.CardBase.CardImg || this.ImgUrl; //卡背景图片
					}
					this.IsOpenSpread = data.Data.IsOpenSpread;
					this.UserPhoto = data.Data.Img; //头像					
					this.data = data.Data.MyCard || {};
					this.AwaitPayCnt = data.Data.AwaitPayCnt;
					this.LeaderAudit = data.Data.LeaderAudit;
			...
			toGrid(val) {
				...
				// } 
					...
					this.$router.push('/pages/vip/markPromotion/markPromotion')
				} else if (id == '6') {
			...
			clickClear() { //清除缓存		
				...
				Cookie.remove("localShop")
				Cookie.remove("currentLocation")
				Cookie.remove("currentStoreInfo")
				Cookie.remove("GroupUrl")
				Cookie.remove(GetAppNo()+"_location")
