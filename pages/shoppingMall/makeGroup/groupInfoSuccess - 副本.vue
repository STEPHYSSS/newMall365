...
<script>
	...
	export default {
		...
		/*  流程梳理：
			1.如果活动剩余时间也为0 的话 就要告诉用户活动已结束并且不展示下方的按钮
			2.如果是个人开的团 个人拼团有效期为0 并且 不是虚拟成团 就展示重新开团 并且不要传递GroupSID
			3.如果是个人开的团 个人有效期为0 并且 是虚拟成团 就要变成拼团成功
		*/ 
		created() {
			...
			async getMyGroupInfo(){//我的团
				...
				try {
					let {
						...
						Action: 'MyPromotion',
						GroupSID:GroupSID?GroupSID:this.GroupSID,
						SID:this.historyUrlSID.query.SID?this.historyUrlSID.query.SID:this.promtionSID,
