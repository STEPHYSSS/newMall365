	...
	<div class="pictureAds">
		...
		<div class="zent-design-preview-controller__drag-handle" style="background-color: rgb(249, 249, 249);">
          ...
          currentObj.imgRadius==='1'?'cap-image-ad--fillet':'']">
						...
						<li class="cap-image-ad__content" :style="{ 'margin-bottom': currentObj.imgGap+'px'}"
						 v-for="(item,index) in currentObj.imgList" :key="index" @click="clickLink(item)">
							...
							<div class="cap-image-ad__image-wrapper">
								<!-- #ifndef H5-->
								<image class="cap-image-ad__image" :src="item.img " alt="loaded" />
								<!-- #endif -->
								<!-- #ifdef H5 -->
					...
					<swiper v-if="currentObj.changeMode==2"  :style="{'height':imgInfo.height+'px','width':imgInfo.width<1000?changeClientWidth+'px':'auto'}"
					 @change="onChange" :indicator-dots="currentObj.indicator==1&&currentObj.imgList.length>1">
						<swiper-item v-for="(image, index) in currentObj.imgList" :key="index" @click="clickLink(image)">
							<!-- #ifndef H5-->
							<image :src="image.img" style="width:100%;" mode="scaleToFill"/>
							<!-- <a :href = "image.urlObj.url"></a> -->
							<!-- #endif -->
							<!-- #ifdef H5 -->
							<img :src="image.img " style="width:100%;" mode="scaleToFill"/>
							<!-- #endif -->
		...
		mounted() {
			...
			this.currentObj.imgList.forEach(D => {
				...
				img.onload = function(){
					...
					if(img.height>=400){
						vm.$set(vm.imgInfo,'height',Math.ceil(img.height - '368'))		
					}else{
					...
					if(img.width>=1000){
						...
						this.changeClientWidth = img.width
					}
				}
			})
			this.setImgWidth();
