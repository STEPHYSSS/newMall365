<template>
	<view class='container'>
		<div style="height:55px;">
			<uni-search-bar :radius="100" v-model="searchBar" @input="addressSearch" @cancel="canceladdressSearch"></uni-search-bar>
		</div>
		<div v-if="!showSearch">
			<map id='map' :scale='map.scale' :show-location='map.showLocation' :longitude='map.longitude' :latitude='map.latitude'
			 :width='map.width' :height='map.height' :markers='map.markers' @regionchange='mapChange'>
		...
		<div v-if="showSearch">
			<view class='bg-ff'>
				...
				<view class='padding border-t list position-r' v-for='(item, index) in listArea' :key='index' @click='addressFun(item)'>
		...
		methods: {
			...
			mapChange(e) {
				...
				this.timer = setTimeout(() => {
					if (e.type == 'end') {
						that.mapCtx = uni.createMapContext('map', that)
